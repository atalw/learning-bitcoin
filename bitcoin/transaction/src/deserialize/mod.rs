...
impl Transaction {
	pub fn from(raw_transaction: String) -> Result<Transaction, Box<dyn Error>> {
		let raw_transaction = match serde_json::from_str::<Value>(&raw_transaction) {
			Ok(d) => d["result"].to_string(),
			Err(_) => raw_transaction
		};
