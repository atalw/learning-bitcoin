...
fn main() {
	...
	// match parse_raw_data(data_pre_segwit, false) {
	//     Ok(()) => {},
	//     Err(e) => panic!("{}", e)
	// }

	match parse_raw_data(data_segwit, true) {
		Ok(()) => {},
		Err(e) => panic!("{}", e)
	}
}

fn parse_raw_data(data: String, segwit: bool) -> Result<(), Box<dyn Error>> {
	...
	for i in 0..number_of_inputs {
		...
		let out_script_length = get_decimal_value(&out_script_length_bytes) as usize;
